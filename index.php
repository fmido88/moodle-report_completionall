 ...
 *
 * Copied from report_completion and modified
...
$url = new moodle_url('/report/completionall/index.php', ['course' => $course->id, 'enrolstat' => $enrolstat]);
$PAGE->set_url($url);
...
$leftcols = 2 + count($extrafields);

// Check permissions.
require_login($course);

require_capability('report/completionall:view', $context);
...
if ($csv) {
    ...
    $pluginname = get_string('pluginname', 'report_completionall');
...
if (!$csv) {
    ...
    echo '<th scope="col" class="completion-identifyfield">' .
            get_string('enrol_status', 'report_completionall') . '</th>';
        ...
        switch ($criterion->criteriatype) {
            ...
            case COMPLETION_CRITERIA_TYPE_COURSE:
                ...
                $iconattributes['title'] = format_string($crs->fullname, true, [
                                                'context' => context_course::instance($crs->id, MUST_EXIST),
                                            ]);
                $iconalt = format_string($crs->shortname, true, ['context' => context_course::instance($crs->id)]);
                break;
    ...
    $row[] = get_string('id', 'report_completionall');
    $row[] = get_string('name', 'report_completionall');
    foreach ($extrafields as $field) {
        $row[] = \core_user\fields::get_display_name($field);
    }
    $row[] = get_string('enrol_status', 'report_completionall');
    ...
    foreach ($criteria as $criterion) {
        ...
        if ($criterion->criteriatype == COMPLETION_CRITERIA_TYPE_ACTIVITY) {
            ...
            $row[] = $formattedname = format_string($mod->name, true,
                    ['context' => context_module::instance($criterion->moduleinstance)]);
            $row[] = $formattedname . ' - ' . get_string('completiondate', 'report_completionall');
        } else {
...
foreach ($progress as $user) {
    $enrolstatus = get_string('enrol_'.$user->enrolstatus, 'report_completionall');
    // User name.
    if ($csv) {
        $row = [];
        $row[] = $user->id;
        ...
        $row[] = clean_param($enrolstatus, PARAM_NOTAGS);
    } else {
        print PHP_EOL.'<tr id="user-'.$user->id.'">';

        if (completion_can_view_data($user->id, $course) && $user->enrolstatus == 'active') {
            $userurl = new moodle_url('/blocks/completionstatus/details.php', ['course' => $course->id, 'user' => $user->id]);
        } else {
        ...
        if ($csv) {
            ...
            if ($allowmarkingcriteria === $criterion->id) {
                ...
                $toggleurl = new moodle_url(
                    '/report/completionall/togglecompletion.php',
                    [
                        'user' => $user->id,
                        'course' => $course->id,
                ...
                print '<a href="'.$toggleurl->out().'" title="'
                    .s(get_string('clicktomarkusercomplete', 'report_completionall')).'">' .
...
$event = \report_completionall\event\report_viewed::create(['context' => $context]);
$event->trigger();
