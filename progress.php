 ...
 *
 * copied from plugin report_progress and modified.
...
/**
 * Add a quote for csv of excel file.
 * @param string $value
 * @return string
 */
...
$url = new moodle_url('/report/completionall/progress.php', ['course' => $id, 'enrolstat' => $enrolstat]);
if ($sort !== '') {
    $url->param('sort', $sort);
}
if ($format !== '') {
...
if ($csv && $grandtotal && count($activities) > 0) { // Only show CSV if there are some users/actvs.
    ...
    $pluginname = get_string('progress_report', 'report_completionall');
...
$link = $CFG->wwwroot.'/report/completionall/progress.php?course='.$course->id;
if (strlen($sort)) {
    $link .= '&amp;sort='.$sort;
}
$link .= '&amp;start=';
...
if (!$csv) {
    ...
    echo $sep . csv_quote(get_string('enrol_status', 'report_completionall'));
}
...
foreach ($progress as $user) {
    ...
    if ($csv) {
        ...
        echo $sep . csv_quote(clean_param($enrolstatus, PARAM_NOTAGS));
    } else {
