 ...
 *
 * Toggles the manual completion flag for a particular activity
 * or course completion and the current user.
 * If by student params: course=2
 * If by manager params: course=2&user=4&rolec=3&sesskey=ghfgsdf
 *
 * Copied from moodle core package 'Course'
 * @copyright 2023 Mohammad Farouk, copied from core/course.
...
if ($courseid) {
    ...
    if ($user && $rolec) {
        ...
        completion_criteria::factory(['id' => $rolec, 'criteriatype' => COMPLETION_CRITERIA_TYPE_ROLE]);
        ...
        $buttoncontinue = new single_button(new moodle_url('/course/togglecompletion.php',
                                            ...
                                            get_string('yes'), 'post');
        $buttoncancel   = new single_button(new moodle_url('/course/view.php', ['id' => $courseid]), get_string('no'), 'get');
